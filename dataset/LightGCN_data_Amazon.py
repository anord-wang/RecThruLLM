...
def split_user_item_interactions(interactions):
    ...
    for user_id, item_list in interactions.items():
        ...
        num_test = max(1, int(len(item_list) * 0.1))
        num_train = int(len(item_list)) - num_test

        # Split the shuffled item interactions list
        train_items = item_list[:num_train]
        test_items = item_list[num_train:num_train + num_test]
...
def save_txt_file(train_interactions, test_interactions, data_name):
    train_dataset_path = os.path.join(data_name, "train.txt")
    with open(train_dataset_path, 'a') as f:
        for index in range(len(train_interactions)):  # 使用 range() 获取正确的索引
            # 使用空格或其他分隔符拼接每个元素，然后转换整个列表为字符串
            line = str(index) + ' ' + ' '.join([str(t) for t in train_interactions[index]]) + '\n'
            f.write(line)
    test_dataset_path = os.path.join(data_name, "test.txt")
    with open(test_dataset_path, 'a') as f:
        for index in range(len(test_interactions)):  # 使用 range() 获取正确的索引
            # 使用空格或其他分隔符拼接每个元素，然后转换整个列表为字符串
            line = str(index) + ' ' + ' '.join([str(t) for t in test_interactions[index]]) + '\n'
            f.write(line)

            
def get_user_item_list(data_maps, data_name):
    data_maps["user2id"]
    data_maps["item2id"]
    
    user_list_dataset_path = os.path.join(data_name, "user_list.txt")
    item_list_dataset_path = os.path.join(data_name, "item_list.txt")

    with open(user_list_dataset_path, 'w') as file:
        # 写入列标题
        file.write("org_id remap_id\n")
        # 遍历字典，写入数据
        for key, value in data_maps["user2id"].items():
            line = f"{key} {value}\n"
            file.write(line)
    
    with open(item_list_dataset_path, 'w') as file:
        # 写入列标题
        file.write("org_id remap_id\n")
        # 遍历字典，写入数据
        for key, value in data_maps["item2id"].items():
            line = f"{key} {value}\n"
            file.write(line)
...
train_interactions, test_interactions = split_user_item_interactions(user_items)
save_txt_file(train_interactions, test_interactions, short_data_name)
get_user_item_list(datamaps, short_data_name)
