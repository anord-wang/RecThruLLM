...
def Amazon(dataset_name, rating_score):
    ...
    # data_file = './dataset/raw_data/' + dataset_name + '.json.gz'
    for record in parse(data_file):
        ...
        if float(record['overall']) < rating_score:
            datas_negative.append((user, item, int(time)))
            datas_else.append((user, item, int(time)))
        elif float(record['overall']) == rating_score:
            datas_else.append((user, item, int(time)))
        # continue
        else:
        ...
        data_reviews.append((user, item, review, score))
    # print(f'{dataset_name} data has been loaded! Total {len(datas)} records!')
    # print(f'Negative data has been loaded! Total {len(datas_negative)} records!')
    # print(f'Else data has been loaded! Total {len(datas_else)} records!')
    return datas, datas_negative, datas_else, data_reviews
...
def Amazon_meta(dataset_name, data_maps):
    ...
    for info in parse(meta_file):
        if 'title' in info:
            ...
            item_details[info['asin']] = name, description
        else:
            continue

    return datas, item_details
...
def main(data_name, acronym, data_type='Amazon'):
    ...
    if data_type == 'Yelp':
        ...
        # datas, datas_negative, datas_else = Amazon(data_name+'_5', rating_score=rating_score)
        datas, datas_negative, datas_else, data_review = Amazon(data_name, rating_score=rating_score)

    user_items = get_interaction(datas)
    print('interaction length', len(user_items))
    ...
    meta_infos, item_details = Amazon_meta(data_name, data_maps)
    ...
    return meta_infos, user_items, item2attrs, datamaps, datas_negative, datas_else, data_review, item_details


# In[34]:


meta_infos, user_items, item2attrs, datamaps, datas_negative, datas_else, data_review, item_details = main(full_data_name, short_data_name, data_type='Amazon')
...
def populate_matrix(train_matrix_shape, data, data_maps):
    print('1', train_matrix_shape)
    matrix = lil_matrix(train_matrix_shape)
    print('2',matrix.shape)
    sum = 0
    for user, item, _ in data:
        user_id = data_maps["user2id"].get(user)
        item_id = data_maps["item2id"].get(item)
        # print(f"Populating matrix: user_{user_id} -> item_{item_id}")
        if user_id is not None and item_id is not None:
            matrix[user_id, item_id] = 1.0
            sum = sum + 1
            # print(f"Populating matrix: user_{user_id} -> item_{item_id}")
    print(f"Populated {sum} interactions")
    print('3',matrix.shape)
    print('4',matrix.tocsr().shape)
    return matrix.tocsr()

def score_prediction_dataset(data, data_maps, data_name):
    dataset = []
    for user, item, review, score in data:
        user_id = data_maps["user2id"].get(user)
        item_id = data_maps["item2id"].get(item)
        if user_id is not None and item_id is not None:
            text = f"user_{user_id} write a review for item_{item_id}, the review is: {review}"
            if score < 3:
                label = 0
            elif score == 3:
                label = 1
            ...
            line = f"{text}\t{label}\n"
            dataset.append(line)
    dataset_path = os.path.join(data_name, "score_prediction.txt")
    # if not os.path.exists(os.path.join("./dataset", data_name, "score_prediction.txt")):
        # os.makedirs(os.path.join("./dataset", data_name, "score_prediction.txt"))
    with open (dataset_path, 'w', encoding='utf-8') as file:
        for line in dataset:
            file.write(line)

def contrast_learning_dataset(train_matrix, val_matrix, test_matrix, item_details, data_maps, data_name):
    combined_matrix = train_matrix + val_matrix + test_matrix
    combined_matrix[combined_matrix > 1] = 1

    # user
    user_dataset = []
    for user_id in range(combined_matrix.shape[0]):
        # select combined_matrix.shape[1]**2/2500 items pairs randomly without overlap
        for item_id in random.sample(range(combined_matrix.shape[1]), combined_matrix.shape[1]//50):
            for i in random.sample(range(combined_matrix.shape[1]), combined_matrix.shape[1]//50):
                if item_id == i:
                    pass
                item_1 = data_maps["id2item"].get(item_id)
                item_2 = data_maps["id2item"].get(i)
                name_1, description_1 = item_details.get(item_1, (None, None))
                name_2, description_2 = item_details.get(item_2, (None, None))
                if name_1 is not None and name_2 is not None and description_1 is not None and description_2 is not None:
                    text = f"user_{user_id} bought item_{item_id} and item_{i}, item_{item_id} is {name_1}, item_{i} is {name_2}, the description of item_{item_id} is {description_1}, the description of item_{i} is {description_2}."
                    if combined_matrix[user_id, item_id] == 1 and combined_matrix[user_id, i] == 1:
                        label = 1
                        line = f"{text}\t{label}\n"
                        user_dataset.append(line)
                    elif (combined_matrix[user_id, item_id] == 1 and combined_matrix[user_id, i] == 0) or (combined_matrix[user_id, item_id] == 0 and combined_matrix[user_id, i] == 1):
                        label = 0
                        line = f"{text}\t{label}\n"                
                        user_dataset.append(line)
                    else:
                        continue
    user_dataset_path = os.path.join(data_name, "contrast_learning_user.txt")
    # if not os.path.exists(os.path.join("./dataset", data_name, "contrast_learning_user.txt")):
        # os.makedirs(os.path.join("./dataset", data_name, "contrast_learning_user.txt"))
    with open(user_dataset_path, 'a', encoding='utf-8') as file:
        for line in user_dataset:
            file.write(line)


    # item
    item_dataset = []
    for item_id in range(combined_matrix.shape[1]):
        for user_id in random.sample(range(combined_matrix.shape[0]), combined_matrix.shape[0]//50):
            for i in random.sample(range(combined_matrix.shape[0]), combined_matrix.shape[0]//50):
                if user_id == i:
                    pass
                name, description = item_details.get(data_maps["id2item"].get(item_id), (None, None))
                if name is not None and description is not None:
                    text = f"user_{user_id} and user_{i} both bought item_{item_id}, item_{item_id} is {name}, the description of item_{item_id} is {description}."
                    if combined_matrix[user_id, item_id] == 1 and combined_matrix[i, item_id] == 1:
                        label = 1
                        line = f"{text}\t{label}\n"
                        item_dataset.append(line)
                    elif (combined_matrix[user_id, item_id] == 1 and combined_matrix[i, item_id] == 0) or (combined_matrix[user_id, item_id] == 0 and combined_matrix[i, item_id] == 1):
                        label = 0
                        line = f"{text}\t{label}\n"
                        item_dataset.append(line)
                    else:
                        continue
    item_dataset_path = os.path.join(data_name, "contrast_learning_item.txt")
    with open(item_dataset_path, 'a', encoding='utf-8') as file:
        for line in item_dataset:
            file.write(line)
...
# construct the review text classification dataset
score_prediction_dataset(data_review, datamaps, short_data_name)
contrast_learning_dataset(train_matrix, val_matrix, test_matrix, item_details, datamaps, short_data_name)


# print(f"datas length: {len(datas_negative)}, {len(datas_else)}")
# # val_matrix_negative, val_matrix_else = negetive_relation_mine(val_matrix, datas_negative, datas_else, data_maps)
# train_matrix_negative = populate_matrix(train_matrix.shape, datas_negative, datamaps)
# train_matrix_else = populate_matrix(train_matrix.shape, datas_else, datamaps)
# # val_matrix_negative = populate_matrix(val_matrix.shape, datas_negative, data_maps)
# # val_matrix_else = populate_matrix(val_matrix.shape, datas_else, data_maps)
...
# save_npz(os.path.join(data_root, 'train_matrix_negative.npz'), train_matrix_negative)
# save_npz(os.path.join(data_root, 'train_matrix_else.npz'), train_matrix_else)
# # save_npz(os.path.join(data_root, 'val_matrix_negative.npz'), val_matrix_negative)
# # save_npz(os.path.join(data_root, 'val_matrix_else.npz'), val_matrix_else)
