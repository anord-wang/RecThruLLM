...
import sys
sys.path.append("/home/local/ASURITE/xwang735/LLM4REC/LLM4RecAgent/")

from newtransformers.models.gpt2 import GPT2ModelABC, GPT2Config
...
local_root = '/home/local/ASURITE/xwang735/LLM4REC/LLM4RecAgent/model_save/beauty_model_save'
# local_root = "tmp"
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
def main():
    ...
    parser.add_argument("--lambda_V", type=str, default=1,
        help="specify the dataset for experiment")
    ...
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    remote_node_center_path = os.path.join(data_root, "kmeans_cluster.pkl")
    local_node_center_path = os.path.join(local_root, "kmeans_cluster.pkl")
    if accelerator.is_main_process:
        save_local(remote_node_center_path, local_node_center_path, "rb", "wb")
    accelerator.wait_for_everyone()
    with open(local_node_center_path, 'rb') as f:
        loaded_data = pickle.load(f)
    a_labels = loaded_data['Labels']
    a_centers = loaded_data['Centers']
    a_labels_gpu = [torch.tensor(labels).to(device) for labels in a_labels]
    a_centers_gpu = [torch.tensor(centers).to(device) for centers in a_centers]
    a_labels = a_labels_gpu
    a_centers = a_centers_gpu
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    remote_train_neg_mat_path = os.path.join(data_root, "train_matrix_negative.npz")
    local_train_neg_mat_path = os.path.join(local_root, "train_matrix_negative.npz")
    # remote_train_neg_mat_path = os.path.join(data_root, "train_matrix_else.npz")
    # local_train_neg_mat_path = os.path.join(local_root, "train_matrix_else.npz")
    if accelerator.is_main_process:
        save_local(remote_train_neg_mat_path, local_train_neg_mat_path, "rb", "wb")
    accelerator.wait_for_everyone()
    
    # Get the training data generator
    train_mat = load_npz(local_train_mat_path)
    train_mat_neg = load_npz(local_train_neg_mat_path)
    print('train_mat.shape: ', train_mat.shape)
    print('train_mat_neg.shape: ', train_mat_neg.shape)
    train_data_gen = RecommendationGPTTrainGeneratorBatch(tokenizer, train_mat, train_mat_neg, mapping_graph_bc)

    # Get the validation data generator
    val_mat = load_npz(local_val_mat_path)
    val_data_gen = RecommendationGPTTestGeneratorBatch(tokenizer, train_mat, train_mat_neg, val_mat, mapping_graph_bc)
    ...
    content_base_model = GPT4RecommendationBaseModel(config, gpt2model, a_centers, a_labels)
    
    # pretrained_root = os.path.join(server_root, "model", dataset, "content")
    # 
    # content模型修改所调用的pt参数
    # pretrained_root = os.path.join(server_root, "model", dataset+'_ainitialization', "content")
    pretrained_root = os.path.join(server_root, "model", dataset, "content_final")
    ...
    gpt2model_rec = GPT2ModelABC(config)
    # remote_pretrained_weights_path = os.path.join(server_root, "model", "pretrained", "gpt2", "allgpt_gpt2_content_luxury_1_6.620403289794922_4.bin")
    ...
    base_model = GPT4RecommendationBaseModel(config, gpt2model_rec, a_centers, a_labels)
    # base_model = GPT4RecommendationBaseModel(config, gpt2model)
    # 
    # Recommendation模型修改所调用的collaborative模型的pt参数
    # pretrained_root = os.path.join(server_root, "model", dataset+'_ainitialization', "collaborative")
    pretrained_root = os.path.join(server_root, "model", dataset, "score_prediction")
    remote_pretrained_user_emb_path = os.path.join(pretrained_root, f"user_embeddings_{args.lambda_V}.pt")
    remote_pretrained_item_emb_path = os.path.join(pretrained_root, f"item_embeddings_{args.lambda_V}.pt")
    local_pretrained_user_emb_path = os.path.join(local_root, f"user_embeddings_{args.lambda_V}.pt")
    local_pretrained_item_emb_path = os.path.join(local_root, f"item_embeddings_{args.lambda_V}.pt")
    ...
    rec_model_root = os.path.join(server_root, "model", dataset+'_ainitialization', "rec")
    accelerator.print(f"Weights will be saved to {rec_model_root}!")
    
    # The place to save the content model weights
    content_model_root = os.path.join(server_root, "model", dataset+'_ainitialization', "content")
    ...
    for epoch in range(num_epochs):
        ...
        # for input_ids, target_mat, attention_mask, input_ids_main, graph_bc in train_data_loader:
            ...
            with torch.no_grad():
                content_embeds = torch.cat(
                    (accelerator.unwrap_model(content_model).base_model.embed(input_ids)[0],
                     accelerator.unwrap_model(content_model).base_model.embed(input_ids_main)[0]),
                    dim=1
                ).to(device)
            ...
            # print('regularize_loss: ', regularize_loss)
        ...
        accelerator.wait_for_everyone()
        with torch.no_grad():
            for input_ids, train_mat, target_mat, target_mat_neg, attention_mask, graph_bc in val_data_loader:
                # Move tensors to the correct device
                input_ids = input_ids.to(device)
                train_mat = train_mat.to(device)
                target_mat = target_mat.to(device)
                target_mat_neg = target_mat_neg.to(device)
                attention_mask = attention_mask.to(device)
                graph_bc = graph_bc.to(device)
                ...
                # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                # strong rules by negative connections for generation
                item_scores[target_mat_neg > 0] = -float("inf")
                # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                ...
                # print('val_rec_loss: ', val_rec_loss)
            ...
            #for input_ids_prompt, input_ids_main, attention_mask in progress_bar:
                ...
                with torch.no_grad():
                    rec_embeds = accelerator.unwrap_model(rec_model).\
                                 base_model.embed(input_ids_prompt)[0].to(device)
    ...
    accelerator.print("-----original test Attention-----")
