...
class RecommendationGPTTrainGeneratorBatch(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        past_interactions_neg = self.train_mat_2.getrow(idx).nonzero()[1]
        ...
        if len(past_interactions_neg) > 0:
            input_prompt += f"user_{idx} does not want to interact with {' '.join(['item_' + str(item_id) for item_id in past_interactions_neg if item_id is not None][:min(len(past_interactions_neg), 20)])}"
...
class RecommendationGPTTestGeneratorBatch(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_interactions_neg = self.train_mat_2.getrow(idx).nonzero()[1]
        if self.shuffle:
            random.shuffle(input_interactions)
        ...
        if len(input_interactions_neg) > 0:
            input_prompt += f"user_{idx} does not want to interact with {' '.join(['item_' + str(item_id) for item_id in input_interactions_neg if item_id is not None][:min(len(input_interactions_neg), 20)])}"
        input_prompt += f", user_{idx} will interact with"
        ...
        # Obtain the negtive items
        target_interactions_neg = self.train_mat_2.getrow(idx).nonzero()[1]
        target_matrix_neg = torch.zeros(self.num_items, dtype=torch.float32)
        target_matrix_neg[target_interactions_neg] = 1.0

        return input_prompt, train_matrix, target_matrix, target_matrix_neg
    ...
    def collate_fn(self, batch):
        ...
        prompt_texts, train_matrices, target_matrices, target_matrix_neg = zip(*batch)
        # graph_bc = self.mapping_graph_bc
        # graph_bc = torch.tensor(graph_bc)
        ...
        target_matrices = torch.cat([matrix.unsqueeze(0) for matrix in target_matrices])
        target_matrix_neg = torch.cat([matrix.unsqueeze(0) for matrix in target_matrix_neg])
        ...
        return prompt_ids, train_matrices, target_matrices, target_matrix_neg, attention_mask, graph_bc
