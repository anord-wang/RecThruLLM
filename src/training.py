...
def main():
    ...
    initialized_user_emb_path = os.path.join(server_root, "model", dataset, "score_prediction", f"user_embeddings_{args.lambda_V}.pt")
    initialized_item_emb_path = os.path.join(server_root, "model", dataset, "score_prediction", f"item_embeddings_{args.lambda_V}.pt")
    content_base_model.user_embeddings.load_state_dict(
        torch.load(initialized_user_emb_path, map_location=device))
    content_base_model.item_embeddings.load_state_dict(
        torch.load(initialized_item_emb_path, map_location=device))
    ...
    collaborative_base_model = GPT4RecommendationBaseModel(config, gpt2model, a_centers, a_labels)
    ...
    content_model_root = os.path.join(server_root, "model", dataset+'_initializeation', "content")
    accelerator.print(f"Weights will be saved to {content_model_root}!")

    # The place to save the collaborative model weights
    collaborative_model_root = os.path.join(server_root, "model", dataset+'_initializeation', "collaborative")
        ...
        for input_ids_prompt, input_ids_main, attention_mask, graph_bc_prompt, graph_bc_combined in progress_bar:
            ...
            with torch.no_grad():
                content_embeds = torch.cat(
                    (accelerator.unwrap_model(content_model).base_model.embed(input_ids_prompt)[0],
